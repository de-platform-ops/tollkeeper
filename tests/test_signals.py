...
def store(request, tmp_path):
    if request.param == "sqlite":
        s = _make_sqlite_store(tmp_path)
    else:
        s = _make_dbapi_store(tmp_path)
    yield s
    s.close()
...
class TestDbApiParamstyle:
    def test_qmark_paramstyle(self, tmp_path) -> None:
        ...
        with DbApiSignalStore(conn, paramstyle="qmark") as store:
    ...
    def test_format_paramstyle_rejected_by_sqlite(self, tmp_path) -> None:
        ...
        with DbApiSignalStore(conn, paramstyle="format") as store:
            assert store._ph == "%s"
            assert store._p("SELECT * WHERE x = ?") == "SELECT * WHERE x = %s"

    def test_invalid_paramstyle_raises(self, tmp_path) -> None:
        with contextlib.closing(sqlite3.connect(str(tmp_path / "bad.db"))) as conn:
...
class TestTollkeeperSignalIntegration:
    ...
    def test_audit_pass_writes_signal(self, tmp_path, make_store) -> None:
        backend = FakeBackend()
        with make_store(tmp_path, suffix="_pass") as signal_store:
    ...
    def test_audit_fail_no_signal(self, tmp_path, make_store) -> None:
        backend = FakeBackend()
        with make_store(tmp_path, suffix="_fail") as signal_store:
            with pytest.raises(AuditFailedError):
                Tollkeeper(backend, signal_store=signal_store).table("my_table").audit(
                    [FailingCheck()], on_failure="stop"
                )
    ...
    def test_audit_deletes_old_signal_before_running(self, tmp_path, make_store) -> None:
        backend = FakeBackend()
        with make_store(tmp_path, suffix="_stale") as signal_store:
